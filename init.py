...
replacement_files = [('template_files/avr_files/avr_cross_properties.cfg', 'avr_files/avr_cross_properties.cfg')]

for template_pair in replacement_files:
    with open(template_pair[0], 'r') as template_file:
        if not os.path.exists(os.path.dirname(template_pair[1])):
            os.mkdir(os.path.dirname(template_pair[1]))
